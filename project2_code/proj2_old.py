...
def main_redundant(state, finish, walls):
    ...
    if grid_using:
        h = lambda state,fline,walls: proj2_example.h_walldist(state,fline,walls,grid)
    else:
...
def main(state,finish,walls):
    ...
    # Take the new version of h_proj1, which needs the grid as a 4th argument, and
    ...
    if edist_to_line((x,y),finish) <= 1 and abs(u) <= 2 and abs(v) <= 2:
        ...
        next_for_fsearch = lambda state2: [(s,1) for s in rt.next_states(state2,finish,walls)]
        ...
        for i in range(len(path)):
            if path[i] == state:
                ...
                if abs(n) <= 1 and abs(m) <=1:
                    ...
                    best = state
                    for i in possible:
                        if best == state and h(i, finish, walls) != infinity:
                            best = i
                        elif h(i, finish, walls) < h(best, finish, walls):
                            best = i
                    velocity = best[1]
                break
    try:     
...
def dummy_finish(x, node_count, prunes, frontier, explored, state, choices_file, h):
    """called during a search, to print info"""
    path = getpath(x)
    ...
    for i in range(len(path) - 1):
        if path[i] == state:
            ...
            (n,m) = velocity
            if abs(n) <= 1 and abs(m) <=1:
                velocity = (int(math.copysign((abs(n)+1),n)), int(math.copysign((abs(m)+1),m)))
            print(velocity)
            print(velocity,file=choices_file,flush=True)
            break
...
def searching(s0, next_states, goal_test, strategy, h, choices_file):
    ...
    while frontier: 
        ...
        if goal_test(x.state):
            ...
            return finish(best, node_count, prunes, frontier, explored)
        else:
...
def opponent1(p, z, finish, walls):
	...
	x = p[0] + z[0]
	y = p[1] + z[1]
	ebest = (0,0)                # best error found so far
...
def next_states(state, f_line, walls):
    ...
    for dx in [0,-1,1,-2,2]:
        for dy in [0,-1,1,-2,2]:
            ...
            (e, err) = opponent1(loc, (wx, wy), f_line, walls)
